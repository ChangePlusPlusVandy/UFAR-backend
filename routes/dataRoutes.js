...
dataRouter.post("/:health_zone_id/therapeutic_coverage", (req, res) => {
	...
	functions.getTherapeuticCoverage(
		health_zone_id,
		req.body.startDate,
		req.body.endDate,
		(result, err) => {
			if (err == null) {
				...
				console.log(
					"Error getting forms for health zone: " + err.message + " " + result
				);
				res.status(500).send({
...
dataRouter.post("/:health_zone_id/geographic_coverage", (req, res) => {
	...
	const health_zone_id = req.params.health_zone_id;
	console.log(
		"Received request for geographic coverage data from health zone with id: " +
      health_zone_id
	);
	// good test 1 - 618b21eb8453970bd916764c
	...
	// call helper function
	functions.getGeographicalCoverage(
		health_zone_id,
		req.body.startDate,
		req.body.endDate,
		(result, err) => {
...
dataRouter.post("/:health_zone_id/drugs", (req, res) => {
	...
	console.log(
		"Received request for drug proportion data from health zone with id: " +
      health_zone_id
	);

	// call helper function
	functions
		.getDrugData(health_zone_id, req.body.startDate, req.body.endDate)
		.then((data) => {
			if (data.error != null) {
				...
				res.status(500).send({
					message:
            data.error.message ||
            "Some error occurred while getting the drug data."
				});
			} else {
				console.log(
					"Found and returned drug data from " +
