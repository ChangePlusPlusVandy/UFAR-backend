...
authRouter.post('/update_password', async function (req, res) {
  if (!req.user) {
    res.status(401).send({ message: "User doesn't have required privileges/not authorized." })
    return
  }
  ...
  let username = req.user.user.name // if a normal user is sending request, this will be the username
